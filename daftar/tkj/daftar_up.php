    ...
    if ($_POST['upload']) {
      ...
      if ($_POST['upload']) {
        ...
        if ($_POST['upload']) {
            ...
            if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
                if ($ukuran < 544070) {
                    move_uploaded_file($file_tmp, '../../assets/file_upload/tkj/'.$nisn.'-piagam2'.'.pdf');
                } else {
                    echo 'pdf_piagam2';
                    echo 'UKURAN FILE TERLALU BESAR';
                    exit;
...
// 06 = tpm
$kode_jurusan = '01';
$cek_max = mysqli_query($koneksi, "SELECT max(id) FROM f_siswa_tkj");
...
mysqli_query($koneksi, "INSERT INTO f_siswa_tkj VALUES (
                          '',
                          '$no_p',
                          '$tgl_pendaftaran',
                          '$kompetensi_keahlian',
