  ...
  if ($_POST['upload']) {
    ...
    if ($_POST['upload']) {
      ...
      if ($_POST['upload']) {
          ...
          if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
              if ($ukuran < 544070) {
                  move_uploaded_file($file_tmp, '../../assets/file_upload/tpm/'.$nisn.'-piagam1'.'.pdf');
              } else {
                  echo 'pdf_piagam1';
                  echo 'UKURAN FILE TERLALU BESAR';
                  exit;
        ...
        if ($_POST['upload']) {
            ...
            if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
                if ($ukuran < 544070) {
                    move_uploaded_file($file_tmp, '../../assets/file_upload/tpm/'.$nisn.'-piagam2'.'.pdf');
                } else {
                    echo 'pdf_piagam2';
                    echo 'UKURAN FILE TERLALU BESAR';
                    exit;
          ...
          if ($_POST['upload']) {
              ...
              if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
                  if ($ukuran < 544070) {
                      move_uploaded_file($file_tmp, '../../assets/file_upload/tpm/'.$nisn.'-piagam3'.'.pdf');
                  } else {
                      echo 'pdf_piagam3';
                      echo 'UKURAN FILE TERLALU BESAR';
                      exit;
...
// 06 = tpm
$kode_jurusan = '01';
$cek_max = mysqli_query($koneksi, "SELECT max(id) FROM f_siswa_tpm");
...
mysqli_query($koneksi, "INSERT INTO f_siswa_tpm VALUES (
                          '',
                          '$no_p',
                          '$tgl_pendaftaran',
                          '$kompetensi_keahlian',
