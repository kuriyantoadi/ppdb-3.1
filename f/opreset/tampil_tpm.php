...
<div class="container">
  ...
  <table class="table table-bordered table-hover" id="domainsTable">
    ...
    <tbody>
      ...
      $result = mysqli_query($koneksi, "SELECT * FROM tb_siswa where kompetensi_keahlian='Teknik Pemesinan'");
      $total = mysqli_num_rows($result);
      $pages = ceil($total/$halperpage);

      $data = mysqli_query($koneksi, "SELECT * from tb_siswa where kompetensi_keahlian='Teknik Pemesinan' LIMIT $mulai, $halperpage ");
      $no = $mulai+1;
    ...
    while ($d = mysqli_fetch_array($data)) {
      ...
      <tr>
        ...
        <td>
          <center>
            <a style="margin-right: 10px;" type="button" class="btn btn-warning btn-sm" href="siswa_reset.php?id=<?= $d['id'] ?>&kode_jur=tpm" onclick="return confirm('Anda yakin Reset data siswa <?php echo $d['nama_siswa']; ?> ?')">Reset Data</a>
        </td>
      </tr>
